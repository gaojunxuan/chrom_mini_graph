...
//    let dist_ref = -1.0 * ref_order_dist;
    let dist_query = 100.0 - 2. * query_order_dist;
...
fn get_chains<'a>(
    ...
    if chain_reads {
        return chain_range_vec.iter_mut().map(|x| (mem::take(&mut x.0), x.3)).collect();
    }
    //Return best chain for genome chaining
        ...
        let (mut best_seq_anchors_2, _range_ref, _range_query, second_best_aln_score) =
            mem::take(&mut get_best_chains(
                f,
                pointer_array,
            ...
            for l in 0..usize::min(parent_paths.len(), cutoff) {
                ...
                if parent_path_color & edge_color != 0 {
                    ...
                    if anchor_hit {
                        ...
                        let gap_cost = ((query_dist as i64).abs() - ref_dist as i64).abs();
                        //                            + (query_dist as i64 + ref_dist as i64) / 3;
...
//    }
    ...
    for i in 0..vec.len() {
        ...
        let chain_start_ref = seeds_ref[anchors[chain_sequence[chain_sequence.len() - 1]].0 as usize].order;
